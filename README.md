...
・出品者は自分が出品した商品を購入することが出来ません。
(トップページ、およびマイページからアクセスすると「現在出品中です」のメッセージが表示されます)

・プロフィールが作成されていない状態では商品の購入は出来ません。(出品者が発送する際に必要な購入者の氏名と送り先が存在しないため)

・ログインしていない状態では出品ページにはアクセス不可。(ボタンを非活性にしてあります)

・コメント一覧はコメント作成画面から吹き出しアイコンをクリックすることで閲覧可能。

・コメント投稿はログインユーザーのみ可能ですが、コメント一覧は未ログインでも閲覧可能。

・リプライ投稿はコメント一覧ページから自身が出品した商品にコメントがついた場合のみ「このコメントに返信する」というボタンが表示され、リプライ投稿ページへアクセス可能になります。

・お気に入り登録、及び解除はログインユーザーのみ可能。お気に入りの数は未ログインでも確認可能。
...
![image](https://github.com/user-attachments/assets/acf6a418-63a8-409c-977f-d8e20a61ba78)
...
13.MySQLコンテナからMySQLにrootユーザでログインし,CREATE DATABASE demo_test

14.database.phpを開き、mysqlの配列部分をコピーしてmysql_testを作成し、databaseをdemo_test、usernameをroot、passwordをrootに変更

15.cp .env .env.testing後にAPP_ENV=test、APP_KEY=、DB_DATABASE=demo_test、DB_USERNAME=root、DB_PASSWORD=rootと記述修正しphp artisan key:generate --env=testing

16.php artisan migrate --env=testing

17.php artisan db:seed --env=testing
...
・確認用としてテスト用ユーザ（user_id:1）で商品を1件(item_id:13)購入しています。

・確認用としてseederで登録したユーザー（user_id:2）を利用して、item_id:1の商品にコメントを1件、そのコメントに対してリプライを1件手動で登録してあります。

・確認用として管理者から全ユーザーに向けてメールを1件送信しています。(mailhogから閲覧可能)
